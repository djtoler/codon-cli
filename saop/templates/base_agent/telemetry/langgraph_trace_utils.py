...
# # def initialize_telemetry(service_name: str) -> None:
# #     """Initializes the OpenTelemetry tracing provider and exporter."""
...
#     """Generates a unique ID for an agent based on its name and role."""
#     hasher = hashlib.sha256()
#     hasher.update(f"{name}-{role}".encode("utf-8"))
#     return hasher.hexdigest()


# def enrich_llm_span_with_metadata(span: Span, response: Any, start_time: float, end_time: float) -> None:
#     """Enriches an LLM span with token usage, latency, and model name."""
#     # The FIX: Check if the response object has the required attribute
...
#     else:
#         # If it doesn't have response_metadata, do nothing and return.
#         # This prevents the TypeError.
#         pass

# def track_llm_call(name: str):
#     """
#     Decorator to create a nested span for a direct LLM call.
#     """
#     def decorator(func):
#         @wraps(func)
...
#                 end_time = time.time()
#                 enrich_llm_span_with_metadata(span, result, start_time, end_time)
#                 return result
#         return awrapper
#     return decorator
...
#                     # Use a conditional check before setting the attribute
