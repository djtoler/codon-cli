...
# agent_card_generator.py
"""
Dynamic agent card generation from RoleConfig data and Policy Config.
Converts role definitions and main agent configs into A2A agent cards.
"""
...
class AgentCardMapper:
    ...
    def map_main_agent_to_agent_card(self, agent) -> AgentCard:
        """Convert a MainAgentTemplate to an AgentCard using policy_config data."""
        
        # Get role info from the main agent
        role_info = agent.get_role_info()
        
        # Build skills from tools
        skills = []
        for tool_name in role_info.get('tools', []):
            skills.append(
                AgentSkill(
                    id=f"tool_{tool_name}_{uuid.uuid4().hex[:8]}",
                    name=tool_name,
                    description=self.config.get_tool_description(tool_name),
                    tags=["tool", "main_agent"] + getattr(agent.main_agent_config, 'tags', []),
                )
            )

        # Define extensions for main agent
        extensions: list[AgentExtension] = [
            AgentExtension(
                uri="urn:a2a:extension:main_agent",
                description="Main agent from policy configuration.",
                params={
                    "agent_type": "main_agent",
                    "source": "policy_config",
                    "human_review_required": agent.requires_human_review(),
                    "model_config": getattr(agent.main_agent_config, 'model', {}),
                },
            ),
            AgentExtension(
                uri="urn:a2a:extension:policy_metadata",
                description="Metadata from policy configuration.",
                params={
                    "compliance": getattr(agent.main_agent_config, 'compliance', []),
                    "capabilities": getattr(agent.main_agent_config, 'capabilities', []),
                    "custom_metadata": getattr(agent.main_agent_config, 'metadata', {}),
                },
            ),
        ]
        ...
        # Create AgentCard for main agent
        card = AgentCard(
            name=agent.role_name,
            description=getattr(agent.main_agent_config, 'description', role_info.get('system_prompt', f'Main agent: {agent.role_name}')),
            version=getattr(agent.main_agent_config, 'version', '1.0.0'),
            provider=AgentProvider(
                organization=getattr(agent.main_agent_config, 'owner', 'SAOP Platform'),
                url=getattr(agent.main_agent_config, 'docs_url', 'https://example.com/docs'),
            ),
            documentation_url=getattr(agent.main_agent_config, 'docs_url', None),
            skills=skills,
            capabilities=capabilities,
            default_input_modes=["text/plain", "application/json"],
            default_output_modes=["text/plain", "application/json"],
...
def create_agent_card_for_main_agent(
    agent, config: Optional[AgentCardConfig] = None
) -> AgentCard:
    """Create an agent card for a main agent from policy_config."""
    mapper = AgentCardMapper(config)
    return mapper.map_main_agent_to_agent_card(agent)
...
def generate_agent_card_for_executor(
    ...
    """
    Primary interface for servers to generate agent cards from executors.
    Now supports both main agents (policy_config) and expert roles (roles.py).
    ...
    # Determine agent type and generate accordingly
    try:
        if hasattr(executor, 'agent'):
            # Check if this is a MainAgentTemplate (from policy_config)
            if hasattr(executor.agent, 'main_agent_config'):
                print(f"🤖 Generating dynamic agent card for main agent: {executor.role_name}")
                agent_card = create_agent_card_for_main_agent(executor.agent, card_config)
                print(f"✅ Main agent card generated for '{executor.role_name}'")
                return agent_card
            
            # Check if this is a RoleBasedAgentTemplate (from roles.py)  
            elif hasattr(executor.agent, 'role_config'):
                print(f"🤖 Generating dynamic agent card for expert role: {executor.role_name}")
                agent_card = create_agent_card_for_role(executor.role_name, card_config)
                print(f"✅ Expert role card generated for '{executor.role_name}'")
                return agent_card
        
        # Fallback: try standard role-based generation
        print(f"🤖 Attempting standard role generation for: {executor.role_name}")
        agent_card = create_agent_card_for_role(executor.role_name, card_config)
        print(f"✅ Dynamic agent card generated for '{executor.role_name}'")
        return agent_card
        ...
        # Create a minimal agent card with valid A2A fields
        try:
            minimal_card = AgentCard(
                name=f"Agent ({executor.role_name})",
                description=f"SAOP agent for role: {executor.role_name}",
